...
pub fn draw_step_area(ui: &mut egui::Ui, kbd: &KbdQueue, rect: Rect) {
    ...
    // Full 3-octave view: every step's pitch is visible at all times,
    // bar height = pitch (classic 303-style sequencer).
    const SEMI_LO_ABS: f32 = 24.0;
    const SEMI_HI_ABS: f32 = 60.0;
    let win_lo: f32 = SEMI_LO_ABS;
    let win_hi: f32 = SEMI_HI_ABS;
