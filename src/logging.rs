//! File-based logging for SquelchBox via `tracing`.
//!
...
//! surfaced to `stderr` as a last resort — the plugin must continue to run
...
pub fn init() {
    INIT.call_once(|| {
        if let Err(e) = try_init() {
            eprintln!("SquelchBox: logging init failed: {e}");
        }
    });
}

fn try_init() -> Result<(), String> {
    let dir = paths::squelchbox_log_dir();
    fs::create_dir_all(&dir).map_err(|e| format!("create log dir {dir:?}: {e}"))?;

    let log_path = dir.join("squelchbox.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("squelchbox=debug,squelchbox_standalone=debug")
        } else {
            EnvFilter::new("squelchbox=info,squelchbox_standalone=info")
        }
    });
    ...
    tracing::info!("SquelchBox logger initialized — log file: {:?}", log_path);
    Ok(())
}
...
pub fn log_file_path() -> PathBuf {
    paths::squelchbox_log_dir().join("squelchbox.log")
}
